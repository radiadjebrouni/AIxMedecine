...
def get_sub_volume(image, label, 
    ...
    while tries < max_tries:
        ...
        if bgrd_ratio < background_threshold:
            ...
            X = np.moveaxis(np.moveaxis(np.moveaxis(X,2,3),1,2),0,1)
            ...
            y = np.moveaxis(np.moveaxis(np.moveaxis(y,2,3),1,2),0,1)
...
def dice_coefficient(y_true, y_pred, axis=(1, 2, 3), 
    ...
    dice_numerator = 2 * K.sum((y_true*y_pred),axis) +epsilon
    #(2 * K.sum((y_true[0,:,:,:]*y_pred[0,:,:,:]),axis) +epsilon) + (2 * K.sum((y_true[1,:,:,:]*y_pred[1,:,:,:]),axis) +epsilon) + (2 * K.sum((y_true[2,:,:,:]*y_pred[2,:,:,:]),axis) +epsilon))/3
    dice_denominator = K.sum(y_true,axis)+ K.sum(y_pred,axis)+epsilon
...
def soft_dice_loss(y_true, y_pred, axis=(1, 2, 3), 
    ...
    dice_numerator = 2 * K.sum((y_true *y_pred),axis) +epsilon
    dice_denominator = K.sum(y_true*y_true,axis)+ K.sum(y_pred*y_pred,axis)+epsilon
