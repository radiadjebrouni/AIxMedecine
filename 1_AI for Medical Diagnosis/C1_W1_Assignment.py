...
def check_for_leakage(df1, df2, patient_col):
    ...
    #get unique ids and then check the existance of an intersection
...
def get_weighted_loss(pos_weights, neg_weights, epsilon=1):
    ...
    def weighted_loss(y_true, y_pred):
        ...
        for i in range(len(pos_weights)):
         ...
         loss += K.mean(-(pos_weights*y_true[i]*K.log(y_pred[i]+epsilon)+neg_weights*(1-y_true[i])*K.log(1-y_pred[i]+epsilon)))
